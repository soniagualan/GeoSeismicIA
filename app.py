...
def build_pdf(out_path, logo_left_path, logo_right_path, titulo_reporte, img_original_path, img_resultado_path, texto):
    """Genera un PDF multipágina interpretando Markdown básico."""
...
def colorize_mask(mask):
    if mask.ndim == 3: mask_gray = mask[:, :, 0]
    else: mask_gray = mask
    colored_mask = np.zeros((mask_gray.shape[0], mask_gray.shape[1], 3), dtype=np.uint8)
    class_colors = {
        0: (0, 0, 0), 1: (165, 42, 42), 2: (0, 0, 255), 3: (128, 0, 128),
        4: (245, 222, 179), 5: (255, 165, 0), 6: (255, 255, 0), 7: (0, 255, 255),
        8: (255, 0, 255), 9: (220, 20, 60), 10: (0, 0, 200), 11: (255, 182, 193),
        12: (255, 69, 0), 13: (0, 255, 180), 14: (34, 139, 34),
    }
...
def create_overlay_from_mask(img_original, mask_img, alpha=0.3):
    ...
    overlay = base.copy()
    overlay[mask_area] = ((1 - alpha) * base[mask_area] + alpha * mask[mask_area])
...
.header { background-color: #0B3C5D; padding: 16px; border-radius: 14px; color: white; text-align: center; }
.linea { border-top: 3px solid #0B3C5D; margin: 18px 0; }
.bloque { background-color: #F4F6F8; padding: 18px; border-radius: 12px; }
.titulo_azul { background-color:#0B3C5D; color:white; padding:10px 14px; border-radius:10px; font-weight:bold; }
</style>
...
# --------------------------------------------------
# 6. LÓGICA DE PROCESAMIENTO CON SESSION STATE
# --------------------------------------------------

# A) Limpiar estado si se cambia de archivo
if 'last_filename' not in st.session_state:
    st.session_state['last_filename'] = None

if archivo is not None and archivo.name != st.session_state['last_filename']:
    # Si el usuario subió una imagen nueva, borramos los resultados viejos
    st.session_state['last_filename'] = archivo.name
    if 'analysis_result' in st.session_state:
        del st.session_state['analysis_result']
    if 'img_bytes_original' in st.session_state:
        del st.session_state['img_bytes_original']

# B) Botón de Análisis
if archivo is not None:
    # Solo mostramos el botón si NO hay resultados ya en memoria
        ...
        with st.spinner("Conectando con el Orquestador N8N y generando reporte..."):
            try:
                ...
                if response.status_code != 200:
                    ...
                    st.session_state['img_bytes_original'] = image_bytes
                    st.rerun() # Recargamos para mostrar los resultados desde la memoria

            except Exception as e: st.error(f"Fallo de conexión: {str(e)}")

# C) Mostrar Resultados (Persistente)
if 'analysis_result' in st.session_state and 'img_bytes_original' in st.session_state:
    
    # Recuperamos los datos de la memoria
    result = st.session_state['analysis_result']
    image_bytes = st.session_state['img_bytes_original']
    ...
    try:
        ...
        with col_res1:
            ...
            if mask_b64:
                ...
                # --- BOTÓN DESCARGA IMAGEN (No borrará el reporte) ---
                st.download_button(
                    label="⬇️ Imagen interpretada",
    ...
    except Exception as e: st.error(f"Error procesando resultados guardados: {str(e)}")
